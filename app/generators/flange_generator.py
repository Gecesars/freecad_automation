...
def render_flange_body(spec: PartSpec) -> str:
    ...
    lines = [
        ...
        "cut_z = -2.0",
        "shape = record_shape('Base_Flange_Disc', Part.makeCylinder(outer_radius, thickness, Vector(0, 0, 0), Vector(0, 0, 1)), role='base', operation='create', visible=False)",
        "if center_hole_radius > 0:",
        "    center_cut = Part.makeCylinder(center_hole_radius, cut_height, Vector(0, 0, cut_z), Vector(0, 0, 1))",
        "    shape = apply_cut(shape, center_cut, 'Center_Hole')",
        ...
        "        shape = apply_cut(shape, cut, f'Bolt_Hole_{idx + 1:02d}')",
        "try:",
